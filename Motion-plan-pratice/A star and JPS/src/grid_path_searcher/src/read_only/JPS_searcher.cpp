...
{
    // the funcion is checking obstacle around (curr_idx + expand_dir)
...
void JPSPathFinder::JPSGraphSearch(Eigen::Vector3d start_pt, Eigen::Vector3d end_pt)
{
    ...
    */
    ...
    for(int i = 0; i < 3; i++){
        currentPtr->dir(i) = endPtr->index(i) - startPtr->index(i);
        if( currentPtr->dir(i) != 0)
            currentPtr->dir(i) /= abs( currentPtr->dir(i) );
    }
