...
Vector3d getPosPoly( MatrixXd polyCoeff, int k, double t )
{
    ...
    for ( int dim = 0; dim < 3; dim++) {
        ...
        ret(dim) = coeff.dot(time.reverse());
...
VectorXd timeAllocation( MatrixXd Path)
{ 
    VectorXd time(Path.rows() - 1);     //time segment number
    ...
    */
    ...
    double t_0 = _Vel / _Acc;
    double s_0 = _Acc * t_0 * t_0 ;
    for (int i = 1; i < Path.rows(); i++) {
        double delta_dist = (Path.row(i) - Path.row(i - 1)).norm();
        if (delta_dist > s_0) {
            time[i - 1] = t_0 * 2 + (delta_dist - s_0) / _Vel;
        } else {
            time[i - 1] = sqrt(delta_dist / _Acc);
        }
    }
    return time;
}